...
def union_select(select_components, distinct=False, select_type=TRIPLE_SELECT):
    ...
    for table, whereClause, tableType in select_components:
        ...
        if select_type == COUNT_SELECT:
            ...
            inner_select = expression.select(*cols).where(whereClause).distinct().select_from(table).subquery()
            select_clause = expression.select(functions.count().label('aCount')).select_from(inner_select)
